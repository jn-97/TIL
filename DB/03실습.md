...
```sql
SELECT AVG(height) AS '평균 키', AVG(weight) AS '평균 몸무게'
FROM healthcare
GROUP BY age
...
```sql
SELECT AVG(weight/((height*0.01)*(height*0.01))) AS '평균 BMI'
FROM healthcare
GROUP BY is_drinking, smoking
HAVING is_drinking IS NOT NULL AND smoking IS NOT NULL;
```
```
평균 BMI
----------------
